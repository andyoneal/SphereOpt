Shader "VF Shaders/Dyson Sphere/Dyson Shell Unlit Instanced"
{
    ...
    SubShader
    {
        Pass
        {
            Tags
            {
                "LIGHTMODE" = "FORWARDBASE" "QUEUE" = "Geometry" "RenderType" = "DysonShell"
            }
            Cull Off
            Stencil
            {
            ...
            v2f vert(appdata_part v, uint instanceID : SV_InstanceID)
            {
                ...
                o.state_clock_protoid.y = clockwise;
            ...
            fout frag(v2f i, bool viewingOutwardFacingSide : SV_IsFrontFace, float4 screenPos : SV_POSITION)
            {
                ...
                int prevIndex = (int)polygonBaseIndex + prevIndexOffset; //r2.x
                int nextIndex = (int)polygonBaseIndex + nextIndexOffset; //r2.y
                int nextnextIndex = (int)polygonBaseIndex + nextnextIndexOffset; //r2.z
                ...
                int acuteObtuseNextEdge = sign(angleNextEdge);// (int)(angleNextEdge < 0) - (int)(angleNextEdge > 0); //r0.z
                int acuteObtuseThisEdge = sign(angleThisEdge);// (int)(angleThisEdge < 0) - (int)(angleThisEdge > 0); //r0.w

                int clockwise = round(i.state_clock_protoid.y);

                acuteObtuseNextEdge = clockwise * acuteObtuseNextEdge;
                acuteObtuseThisEdge = clockwise * acuteObtuseThisEdge;
                sideOfPrevEdge = clockwise * sideOfPrevEdge; //r1.w
                sideOfThisEdge = clockwise * sideOfThisEdge; //r2.x
                sideOfNextEdge = clockwise * sideOfNextEdge; //r2.y
                ...
                innerEmissionAnim = saturate(innerEmissionAnim * (2.0 * emissStyled.y + emiss.y));
                ...
                float3 colorOutwardFacing = lerp(colorControl * i.color.xyz, i.color.xyz, 1.0 / (100.0 * _EmissionMultiplier[protoId])); //r9.yzw
                ...
                distToCam = asuint(_Global_IsMenuDemo) > 0.5 ? distToCam : (renderPlace > 0.5 ? 3999.9998 * distToCam : distToCam);
                float scaleMetallic = asuint(_Global_IsMenuDemo) > 0.5 ? 0.1 : (renderPlace > 0.5 ? 0.93 : 0.7);
                float distanceScale = saturate(pow(0.25 * log(1.0 + distToCam) - 1.5, 3.0));
                scaleMetallic = scaleMetallic * (1.0 - distanceScale);
                ...
                emission = lerp(_EmissionMultiplier[protoId] * emission * emissionBrightness, plannedShellColor, 0.8 * isPlanned); //r3.yzw
                ...
                colorControl = state > 0.5 || isPlanned > 0.5 ? 1.0 / _EmissionMultiplier[protoId] : colorControl;
                ...
                float finalAlpha = _EmissionMultiplier[protoId] * colorControl;
