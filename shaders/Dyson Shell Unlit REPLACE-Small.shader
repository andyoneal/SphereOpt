Shader "VF Shaders/Dyson Sphere/Dyson Shell Unlit REPLACE Small" {
  Properties {
  ...
  SubShader {
    ...
    Pass {
      ...
      float _Clockwise;
      float _NodeProgressArr[16];
      float4 _PolygonArr[16]; // points that make up the shape of the polygon. repeated PolyCount times.
      float4 _PolygonNArr[16]; // normals
      sampler2D _NoiseTex;
      sampler2D _MainTex;
